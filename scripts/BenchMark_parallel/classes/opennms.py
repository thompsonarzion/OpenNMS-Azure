...
class Opennms:
    ...
    def getNodes(self, requisition="selfmonitor"):
        ...
        if _result.status_code == 200:
            ...
            print(_result.text)
            self._LogEvent("Failed to get list of nodes")
            raise Exception("Failed to get list of nodes")
    ...
    def listRequestions(self):
        ...
        if _result.status_code != 200:
            ...
            self._LogEvent("Failed to retrieve requestion list")
            raise Exception("Failed to retrieve requestion list")
        output = {}
        output["response"] = _result.json()
        output["elapsed"] = _result.elapsed
    ...
    def getRequisition(self, name="selfmonitor"):
        """
        Get a requisition
        """
        self._LogEvent("Running getRequisition method")
        _req=self.listRequestions()["model-import"]
        for a in _req:
            if a["foreign-source"] == name:
                return a
